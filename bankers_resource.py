        ...
        while not all(finish):
            ...
            for i in range(num_processes):
                if not finish[i] and all([temp_need[i][j] <= work[j] for j in range(num_resources)]):
                    work = [work[j] + temp_allocated[i][j]
                            for j in range(num_resources)]
                    finish[i] = True
                    found = True
            if not found:
...
def resource_request(available, allocated, need, num_resources):
    while True:
        ...
        if is_safe_request(process_id, request):
            ...
            available = [available[i] - request[i]
                         for i in range(num_resources)]
            allocated[process_id] = [allocated[process_id][i] + request[i]
                                     for i in range(num_resources)]
            need[process_id] = [need[process_id][i] - request[i]
                                for i in range(num_resources)]
        else:
...
if __name__ == "__main__":
    bankers_safty()
    resource_request(available, allocated, need, num_resources)
